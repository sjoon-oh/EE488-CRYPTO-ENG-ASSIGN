...
x_list = [int(_) for _ in range(BITMASK + 1)]
g_list = [] # graphs 
r_list = [] # reports
...
for x in x_list:
    ...
    init_x = x
    init_h = int(hashlib.md5(init_x.to_bytes(16, 'big', signed=True)).hexdigest(), 16) & BITMASK
    ...
    if (id(g_elem) != id(graph)): # Case when first hash collides.
        ...
        r_list.append((1, -1)) # Initial hash is inside of a cycle, thus -1.
    ...
    print(f"\rProcessing: {x / BITMASK * 100: 4.2f}%", end="")
...
print(f"----")
print(f"Total {n_comp} components.")

print(f"----")
print(f"Average Tail Length: {avg_tail_len}")
print(f"Maximum Tail Length: {max_tail_len}")
print(f"Minimum Tail Length: {min_tail_len}")

print(f"----")
print(f"Average Cycle Length: {avg_cycl_len}")
print(f"Maximum Cycle Length: {max_cycl_len}")
print(f"Minimum Cycle Length: {min_cycl_len}")   
